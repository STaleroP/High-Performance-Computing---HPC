...
{
    int M = 100, N = 100;
    std::string version_folder = "sections";
    ...
    for (int source = euler; source <= GAUSSIAN; ++source)
    {
        ...
        if (report)
        {
            auto simulate_once = [=]() -> std::vector<double>
            {
                ...
                std::vector<std::vector<double>> T, source_grid, source_local;
                source_local.resize(M + 1, std::vector<double>(N + 1, 0.0));

#pragma omp parallel sections firstprivate(h, k)
                {
#pragma omp section
                    {
...
#pragma omp section
                    {
                        calculate_source(M, N, source_local, h, k, current_source, domain);
                    }
                }
                std::vector<double> resultado;
...
#pragma omp section
            ...
            std::cout << "● Tiempo de inicialización: " << b - a << " segundos\n";
