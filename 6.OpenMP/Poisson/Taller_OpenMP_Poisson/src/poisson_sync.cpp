...
void solve_poisson(std::vector<std::vector<double>> &T,
                   ...
                   std::vector<double> &resultado, bool verbose)
{
...
#pragma omp single
            {
                if (verbose)
                    std::cout << "Inicio de iteración " << iterations + 1 << " (Thread " << omp_get_thread_num() << ")\n";
            }

#pragma omp for nowait reduction(max : delta)
            for (int i = 1; i < M; ++i)
            {
                for (int j = 1; j < N; ++j)
                {
...
#pragma omp critical
            {
                if (verbose)
                    std::cout << "Thread " << omp_get_thread_num() << " completó su trabajo en iteración " << iterations + 1 << "\n";
            }
...
{
    int M = 50, N = 50;
    std::string version_folder = "sync";
    ...
    for (int source = euler; source <= GAUSSIAN; ++source)
    {
        ...
        if (report)
        {
            auto simulate_once = [=]() -> std::vector<double>
            {
                ...
                solve_poisson(T, source_grid, M, N, h, k, current_source, resultado, false);
                return resultado;
            };
        ...
        else
        {
            ...
            std::vector<double> resultado;
            solve_poisson(T, source_grid, M, N, h, k, current_source, resultado, true);
